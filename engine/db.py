...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    # Schema mirrors the relevant subset of `picks` so the calibrator
    # can UNION both tables without special-casing. Generated by
    # scripts/backfill_calibration.py -- one row per (game, bet_type)
    # storing what the model would have said for that game and what
    # actually happened. NOT real picks; never used for tracker stats.
    try:
        conn.execute("""
            CREATE TABLE IF NOT EXISTS calibration_samples (
                ...
                result      TEXT NOT NULL,    -- 'W' / 'L' (no pushes)
                created_at  TEXT DEFAULT (datetime('now')),
                UNIQUE(game_id, bet_type)
            )
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_calsamp_bet_prob "
