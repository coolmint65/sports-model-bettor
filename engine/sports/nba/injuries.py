...
def compute_q1_adjustment(team_id: int, season: int) -> dict:
    ...
    """
    from .db import get_team_injuries, get_team_players, get_conn
    ...
    for inj in injuries:
        ...
        # Day-to-day / questionable / probable carry weight=0 — they
        # don't move the prediction (75-80% historically play) but the
        # InjuriesCard still wants to surface them so the user knows
        # who's flagged. Keep them in out_list with q1_impact=0 so
        # they render but the q1_delta math is unaffected.
        if p is None:
            out_list.append({
                "name": inj["name"],
...
if __name__ == "__main__":
    ...
    from .db import get_all_nba_teams
