...
DB_PATH = Path(__file__).resolve().parents[3] / "data" / "nba.db"
...
def _init_schema(conn: sqlite3.Connection) -> None:
    ...
    -- FULL UNIQUE incl. pick text. Previous partial-on-pending
    -- released after settle and let the recorder spam 25+ Q1_TOTAL
    -- copies per game (user flagged 2026-06-10). pick is in the key
    -- to distinguish line variants (Under 215.5 vs Under 217.5).
    CREATE UNIQUE INDEX IF NOT EXISTS uq_nba_picks_family
        ON nba_picks(date, game_id, bet_type, pick);
    ...
    CREATE TABLE IF NOT EXISTS nba_odds (
        ...
        over_under REAL,
        provider TEXT DEFAULT 'HardRock',
    ...
    -- Per-prediction component probabilities. Mirrors engine/db.py's
    -- MLB schema so engine.prediction_log.log_signals can write here.
    -- Used by empirical_calibration to train on EVERY prediction (not
    -- just fired picks), removing the selection-bias gap.
    ...
    CREATE INDEX IF NOT EXISTS idx_nba_pred_signals_sport_market
        ON prediction_signals(sport, market);
    CREATE INDEX IF NOT EXISTS idx_nba_pred_signals_date
...
def upsert_nba_game(game_id: str, **kwargs) -> None:
    ...
    existing = conn.execute("SELECT 1 FROM nba_games WHERE game_id = ?",
                            (game_id,)).fetchone()
    # Data-quality gate (mirror engine.db.upsert_game): refuse to write
    # a stub row missing team_ids on initial insert. Settler + recorder
    # both join via these columns; NULL silently breaks every
    # downstream path. Updates of existing rows still allow None on
    # individual fields (treated as "leave alone").
    if not existing:
        if kwargs.get("home_team_id") is None or kwargs.get("away_team_id") is None:
            logger.warning(
                "upsert_nba_game refusing stub row: game_id=%s missing "
                "team_ids (home=%s, away=%s)",
                game_id, kwargs.get("home_team_id"), kwargs.get("away_team_id"),
            )
            return
...
# Process-local cache for recent team Q1 stats. The predictor calls
# this per (home, away) per match and the slate has 100+ matches —
# without caching every request runs 200+ identical SELECTs. Cleared
# automatically every 5 minutes.
_TEAM_RECENT_CACHE: dict = {}
_TEAM_RECENT_TTL = 300.0  # 5 minutes


def get_team_q1_stats_recent(team_id: int, season: int,
                              last_n: int = 30) -> dict | None:
    """Recent-window variant of get_team_q1_stats. Computes the same
    fields but only over the last ``last_n`` completed games (defaults
    to 30, matching the adaptive baseline's short_window).

    Why this exists: the all-season `get_team_q1_stats` mixes
    regular-season pace with playoff pace, which doesn't match the
    LEAGUE_AVG_Q1_TOTAL adaptive override (recent ~30 games only).
    Predictions stayed biased because team stats said "we score 29
    Q1 ppg" (regular season) while league_avg said "average Q1 total
    is 27.5 per team" (playoffs). Same window for both fixes that.

    Returns None when the team has no completed games. Does NOT
    upsert — this is a read-only helper used at predict time.
    """
    import time as _time
    cache_key = (team_id, season, int(last_n))
    cached = _TEAM_RECENT_CACHE.get(cache_key)
    if cached and _time.time() - cached["ts"] < _TEAM_RECENT_TTL:
        return cached["stats"]
    ...
    rows = conn.execute("""
        ...
        ORDER BY date DESC
        LIMIT ?
    """, (team_id, team_id, season, int(last_n))).fetchall()
    if not rows:
        _TEAM_RECENT_CACHE[cache_key] = {"stats": None, "ts": _time.time()}
        return None
    ...
    q1_allowed: list = []
    q1_home_scored: list = []
    q1_home_allowed: list = []
    q1_away_scored: list = []
    q1_away_allowed: list = []
    leading_q1 = trailing_q1 = 0

    for g in games:
        is_home = g["home_team_id"] == team_id
        if is_home:
            scored, allowed = g["home_q1"], g["away_q1"]
            q1_home_scored.append(scored)
            q1_home_allowed.append(allowed)
        else:
            scored, allowed = g["away_q1"], g["home_q1"]
            q1_away_scored.append(scored)
            q1_away_allowed.append(allowed)
        q1_scored.append(scored)
        q1_allowed.append(allowed)
    ...
    _TEAM_RECENT_CACHE[cache_key] = {"stats": stats, "ts": _time.time()}
    return stats
