...
def generate_picks(date: str | None = None,
    ...
    # Two-tier cap (see TOP_N_PER_GAME / TOP_N_PER_SPORT in
    # mlb_prop_picks): per-matchup dedup feeds slate-wide ranker.
    slate_winners: list[tuple[dict, str, str, str]] = []
    # NBA's pick_date may differ from target_date if HR's matchup
    # resolves to a game scheduled 1 day off. Track per-entry so
    # settling later finds the right boxscore.
    ...
    for matchup_key, prop_rows in props.items():
        ...
        for prop in prop_rows:
            ...
            if scored is None:
                ...
                continue
            # Data-driven NOPLAY gate. engine.edge_floors auto-derives
            # which (sport, bet_type, direction) cells are bleeding ROI
            # at every edge tier and bans them outright. Banned cells
            # never reach insert_pick — voids selection bias before the
            # ranking + cap stages. Same gate the game-line picker has;
            # without it, NBA Player PRA|under bled -25% ROI on n=20
            # because the prop path didn't consult edge_floors.
            from ...edge_floors import is_noplay as _is_noplay
            pick_text = f"{scored['side']} {scored['line']:g}"
            if _is_noplay("nba", prop.get("bet_type", ""),
                           pick_text=pick_text):
    ...
    top, deltas = _apply_slate_top_n("nba", target_date, slate_winners)
    for entry, game_id, _date, matchup in top:
        best = entry["scored"]
        confidence = _confidence_for(best["edge"])
        pick_text = f"{best['side']} {best['line']:g}"
        # Use the per-entry date stamped at collection time (HR matchup
        # may resolve to a different calendar day than target_date).
