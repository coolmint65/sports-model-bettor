...
def calibrate(days: int = 0) -> dict:
    ...
    from .q1_predict import HOME_Q1_BOOST, LEAGUE_AVG_Q1_TOTAL, Q1_STD_DEV
    ...
    # Drop the mc_constants cache so accessors see fresh values on the
    # next call (no process restart needed).
    try:
        from ...mc_constants import invalidate
        invalidate()
    except Exception:
        pass
...
def calibrate_team(team_abbr: str, days: int = 0) -> dict:
    ...
    """
    from .db import get_nba_team_by_abbr
...
def calibrate_full_game(days: int = 0) -> dict:
    """Fit the full-game MC sim constants from finalized games.

    Companion to ``calibrate()`` (Q1). Writes to nba_model_config:
        full_margin_std_dev, full_total_std_dev, full_home_boost, full_avg_total

    These values feed ``engine.mc_constants.nba_full_*`` accessors used by
    ``engine.nba_predict``. The accessors fall back to the historical
    hardcoded baselines if no row exists yet.
    """
    conn = get_conn()
    if days > 0:
        cutoff = (datetime.now() - timedelta(days=days)).strftime("%Y-%m-%d")
        rows = conn.execute(
            "SELECT home_score, away_score FROM nba_games "
            "WHERE status = 'final' AND home_score IS NOT NULL "
            "  AND away_score IS NOT NULL AND date >= ?",
            (cutoff,),
        ).fetchall()
    else:
        rows = conn.execute(
    ...
    if len(rows) < 50:
        return {"message": f"Not enough finals ({len(rows)}) for full-game calibration"}
    ...
    margin_std = math.sqrt(sum((m - avg_margin) ** 2 for m in margins) / n)
    total_std = math.sqrt(sum((t - avg_total) ** 2 for t in totals) / n)

    _save_config("full_home_boost", avg_margin, conn)
    _save_config("full_avg_total", avg_total, conn)
    _save_config("full_margin_std_dev", margin_std, conn)
    _save_config("full_total_std_dev", total_std, conn)

    # Drop the mc_constants cache so the next predict() call sees the
    # fresh values without a process restart.
    try:
        from ...mc_constants import invalidate
        invalidate()
    except Exception:
    ...
    return {
        "games": n,
        "full_home_boost": round(avg_margin, 2),
        "full_avg_total": round(avg_total, 2),
        "full_margin_std_dev": round(margin_std, 2),
        "full_total_std_dev": round(total_std, 2),
    }
...
if __name__ == "__main__":
    ...
    if team_abbr:
        ...
        print(report)
        full_result = calibrate_full_game(days=days_val)
        print(f"\n{'='*60}")
        print("  Full-Game Constants (fed to engine.mc_constants)")
        print(f"{'='*60}")
        if "message" in full_result:
            print(f"  {full_result['message']}")
        else:
            print(f"  Games:                {full_result['games']}")
            print(f"  Home boost (margin):  {full_result['full_home_boost']:+.2f}")
            print(f"  Avg total:            {full_result['full_avg_total']:.2f}")
            print(f"  Margin std dev:       {full_result['full_margin_std_dev']:.2f}")
            print(f"  Total std dev:        {full_result['full_total_std_dev']:.2f}")
        print(f"{'='*60}")
