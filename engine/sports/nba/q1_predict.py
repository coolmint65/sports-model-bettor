...
def _get_team_data(abbr: str, season: int | None = None) -> dict:
    ...
    """
    from .db import get_nba_team_by_abbr, get_team_q1_stats, get_team_q1_stats_recent
    ...
    # Regime alignment: when LEAGUE_AVG_Q1_TOTAL adaptive override is
    # active (recent ~30-game window), pull team stats from the SAME
    # window so the formula `(home_off × away_def) / league_avg`
    # doesn't mix regular-season pace (team values) with playoff
    # pace (league_avg). Using the all-season cache while league_avg
    # tracks playoffs was the root cause of the 2026-05-01 Under
    # bias (audit task #127).
    try:
        from ...model_overrides import get_override as _get_override
        baseline_overridden = _get_override("nba", "LEAGUE_AVG_Q1_TOTAL") is not None
    except Exception:
        baseline_overridden = False

    if baseline_overridden:
        # Match the adaptive baseline's short_window (30 games).
        q1 = get_team_q1_stats_recent(team["id"], season, last_n=30)
        # Fall back to the cached all-season stats when the team
        # doesn't have 30 recent games on record (early-season etc).
        if not q1:
...
def predict_q1(home_abbr: str, away_abbr: str,
    ...
    # Playoff shrinkage. NB: there's a known structural mismatch where
    # team-level q1_off/q1_def are season-wide (higher pace) but the
    # adaptive baseline override is recent-window (playoff pace) —
    # that combo produces predictions that float ABOVE the actual
    # playoff mean. The static factors here partially compensate,
    # though they don't fully unify the regimes (#127 tracks the
    # proper fix: re-derive team Q1 stats from a recent window).
    ...
    # Always populate out_players for the InjuriesCard display, even
    # when the prediction adjustment is gated off. The 2026-04-12 gate
    # was about not letting the q1_delta hit the model — the user
    # still wants to SEE who's out on the game-detail panel.
    if not backtest:
        try:
            from .injuries import compute_q1_adjustment
            if home.get("team_id"):
                _info = compute_q1_adjustment(home["team_id"], season)
                home_roster_adj["out_players"] = _info.get("out_players", [])
                home_roster_adj["starters_out"] = _info.get("starters_out", 0)
            if away.get("team_id"):
                _info = compute_q1_adjustment(away["team_id"], season)
                away_roster_adj["out_players"] = _info.get("out_players", [])
                away_roster_adj["starters_out"] = _info.get("starters_out", 0)
        except Exception as e:
            logger.warning("Roster info populate failed (%s/%s): %s",
                           home_abbr, away_abbr, e)
    if NBA_ENABLE_ROSTER_ADJUSTMENT and not backtest:
        try:
            from .injuries import compute_q1_adjustment, is_likely_resting_spot
            if home.get("team_id"):
            ...
            # SKIPPED in playoffs — no team rests starters in a playoff
            # game, especially not Game 7 elimination. Pre-fix the
            # is_likely_resting_spot heuristic was firing for both teams
            # in BOS-PHI Game 7 (each team had 0 future regular-season
            # games scheduled because the regular season was over) and
            # applying -6pt total Q1 penalty, which bogusly suppressed
            # the model's Q1 total prediction by ~12% of league average.
            today = et_today_str()
            # `in_playoffs` already in scope from earlier in this
            # function — set by `_is_nba_playoffs(today)` near line 396.
            # No team rests starters mid-playoffs.
            if not in_playoffs:
    ...
    # Pull data-fit sigmas (fall back to module defaults if calibration
    # has never run). Resolved per-call so a fresh calibrate() is picked
    # up after invalidate(), without a process restart.
    from ...mc_constants import nba_q1_margin_std, nba_q1_total_std
    margin_sigma = nba_q1_margin_std()
    total_sigma = nba_q1_total_std()

    # Q1 moneyline probability (home wins Q1 outright)
    q1_ml_home = _norm_cdf(predicted_margin / margin_sigma)
...
from .picks import generate_q1_picks, generate_q1_picks_with_context  # noqa: F401, E402
