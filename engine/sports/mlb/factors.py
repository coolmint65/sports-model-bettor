...
def _compute_first_inning(home_xr: float, away_xr: float,
    ...
    """
    # First-inning share of total scoring. Refit 2026-05-31 from
    # 10,000 half-innings — real share is 0.121, not the 0.105 the
    # model was using. Matches inning_weights[0] in scoring.py.
    first_inning_weight = 0.121
    ...
    # Regress toward MLB baseline. Refit 2026-05-31: empirical
    # P(score in inning 1) per team is 0.289 from 10,000 half-innings
    # of linescore data, so P(NRFI) = (1 - 0.289)^2 ≈ 0.506. Prior
    # baseline of 0.56 was 5pp too high → model thought NRFI happens
    # more often than reality → YRFI was underpriced. That alone
    # explains the -26% 1st INN ROI on the 90-day sample.
    MLB_NRFI_BASELINE = 0.506
    nrfi = MLB_NRFI_BASELINE * 0.45 + nrfi * 0.55
    nrfi = max(0.40, min(0.70, nrfi))  # Hard cap — band shifted with baseline
    yrfi = 1 - nrfi
