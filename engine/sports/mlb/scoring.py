...
MLB_AVG_RPG = 4.49         # Average runs per game per team.
                            # Refit 2026-05-31 from 2528 finals over
                            # the trailing 365 days: per-team mean was
                            # 4.489 (home 4.533, away 4.444). Prior
                            # value of 4.85 was overestimating by ~8%
                            # and pushing Over picks too hot — main
                            # driver of the -15% O/U and -24% ALT O/U
                            # ROI seen on the trailing 90-day sample.

# Per-team Negative Binomial dispersion. Refit 2026-05-31 against the
# same trailing-365-day sample: per-team variance averaged 10.32 at
# mean 4.489, giving k = mu^2 / (var - mu) = 3.456. Prior 3.77 was
# slightly too narrow which compounded with the inflated RPG above.
# Variance under NegBin: var = mu + mu^2/k. With mu=4.49, k=3.46 ->
# 10.31, matching observed.
MLB_NEGBIN_K = 3.46
MLB_AVG_ERA = 4.10
MLB_AVG_OPS = .720
MLB_AVG_FIP = 4.10
...
def _compute_f5(home_xr: float, away_xr: float,
    ...
    # Over/Under distribution across typical F5 lines (3.5-7.5)
    ou_lines: dict = {}
...
def _inning_breakdown(home_xr: float, away_xr: float) -> list[dict]:
    """Expected runs by inning, fitted from MLB linescores.

    Refit 2026-05-31 from 10,000 half-innings (5052 games, 2 years).
    Prior hardcoded shape [0.105, 0.100, ..., 0.115, 0.120, 0.140]
    assumed scoring CLIMBS through the game — actual data shows the
    opposite: 9th-inning scoring is the LOWEST (often half-played for
    home teams). Old shape understated 1st (0.105 vs real 0.121) AND
    overstated 9th (0.140 vs real 0.082) by ~70%; this drove the -26%
    1st INN ROI and the late-inning derivative misprices."""
    weights = [0.1207, 0.1038, 0.1157, 0.1191, 0.1193,
                0.1163, 0.1093, 0.1141, 0.0817]
    return [{
        "inning": i + 1,
