...
def _try_score(picks: list, *, type_: str, pick_text: str,
                raw_prob: float, ml,
                direction: str | None = None) -> None:
    """Build a candidate and route through picks_core.score_pick so
    every derivative pick gets the same gates as ML/SPREAD/TOTAL:
    juice wall, plus-odds cap, belief gate (raw_prob >= 0.50),
    empirical + blend calibration, edge ceiling, edge floor.

    Earlier these markets bypassed picks_core entirely -- a positive
    edge from over-confident model probs was enough to ship a pick on
    a side the model expected to LOSE. With this routing, the belief
    gate alone filters those out.
    """
    ...
    try:
        scored = score_pick(candidate, sport="mlb", juice_wall=JUICE_WALL)
    except Exception as e:
        logger.warning("mlb derivative score_pick failed for %s/%s: %s",
                       type_, pick_text, e)
        return
    ...
    for side, abbr, key in (
        ...
        for direction, prob, ml in (
            ...
            _try_score(
                picks,
                type_="Team Total",
                pick_text=f"{abbr} {direction} {line}",
                raw_prob=prob, ml=ml,
    ...
    for period_key, mkt in market.items():
        ...
        for direction, prob, ml in (
            ...
            _try_score(
                picks,
                type_="Inning Total",
                pick_text=f"Inn {period_key} {direction} {line}",
                raw_prob=prob, ml=ml,
                direction=direction.lower(),
            )
    ...
    for period_key, mkt in market.items():
        ...
        for direction, prob, ml in (
            ...
            _try_score(
                picks,
                type_="Inning BTS",
                pick_text=f"Inn {period_key} BTS {direction}",
                raw_prob=prob, ml=ml,
                direction=direction.lower(),
            )
...
def _append_inning_winner(picks: list, pred: dict, odds: dict,
    ...
    for label, prob, ml in (
        ...
        _try_score(
            picks,
            type_="1st Inn Winner",
            pick_text=f"1st Inn {label}",
            raw_prob=prob, ml=ml,
        )
...
def _append_extra_innings(picks: list, pred: dict, odds: dict,
    ...
    for label, prob, ml in (
        ...
        _try_score(
            picks,
            type_="Extra Innings",
            pick_text=f"Extra Innings {label}",
            raw_prob=prob, ml=ml,
            direction=label.lower(),
        )
