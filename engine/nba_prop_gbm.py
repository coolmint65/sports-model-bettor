"""
NBA prop GBM loader (Phase 2k-i for basketball).

Same lazy-load pattern as engine.mlb_prop_gbm. Today only `blk`
shipped (+10.5% MAE reduction). Other stats fall back to rolling-
mean μ in build_player_mc.
"""
...
from .nba_prop_features import feature_cols, extract_features
...
def _model_path(stat_key: str) -> Path:
    return _MODEL_DIR / f"nba_prop_{stat_key}_xgb_latest.json"
...
def _load_model(stat_key: str):
    ...
    try:
        ...
        logger.warning("failed to load NBA GBM for %s: %s", stat_key, e)
        _model_cache[stat_key] = None
        return None
...
def predict_mu(stat_key: str, player_id: int, game_id: str,
    ...
    try:
        ...
        logger.warning("NBA GBM predict failed for %s p=%s: %s",
