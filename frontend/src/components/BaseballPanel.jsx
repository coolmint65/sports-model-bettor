/**
 * BaseballPanel — landing for baseball-framework leagues (College
 * Baseball today; MLB stays on its dedicated heavier path; KBO / NPB
 * could register here later).
 *
 * Line-for-line mirror of FootballPanel since the model architecture
 * is identical (Elo + MC + GBM ensemble, ML/SPREAD/TOTAL markets).
 * "TOTAL" label switched to "Total Runs" / "Run Line" in the odds
 * grid to match baseball convention.
 */
...
export default function BaseballPanel({ api, leagueKey = 'college' }) {
  const [cfg, setCfg] = useState(null)
  const [view, setView] = useState('bets')

  useEffect(() => {
    api.get('/baseball/leagues')
      .then(r => {
  ...
  if (!leagueKey || !cfg) {
    return (
      <PanelShell title="Baseball" tabs={BB_TABS} active={view} onTabChange={setView}>
...
function StandingsView({ api, leagueKey }) {
  ...
  useEffect(() => {
    setLoading(true)
    api.get(`/baseball/${leagueKey}/standings`)
      ...
      <table className="w-full text-sm">
        <thead className="bg-muted/40">
          <tr className="text-[10px] uppercase tracking-wider text-muted-foreground">
            ...
            <th className="text-right px-4 py-2">RF</th>
            <th className="text-right px-4 py-2">RA</th>
        ...
        <tbody>
          {rows.map((t, i) => (
            <tr key={t.team_id ?? i}
              ...
              <td className="px-4 py-2 text-right tabular-nums">{t.runs_for ?? 0}</td>
              <td className="px-4 py-2 text-right tabular-nums">{t.runs_against ?? 0}</td>
              <td className={cn('px-4 py-2 text-right tabular-nums',
                (t.run_diff ?? 0) > 0 ? 'text-positive'
                : (t.run_diff ?? 0) < 0 ? 'text-negative' : '')}>
                {(t.run_diff ?? 0) > 0 ? '+' : ''}{t.run_diff ?? 0}
              </td>
...
function BetsView({ api, leagueKey, cfg }) {
  ...
  const refresh = () => {
    setLoading(true)
    setErr(null)
    api.get(`/baseball/${leagueKey}/today`)
        ...
        <>
          <div className="flex items-center justify-between">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {slate.date}{slate.fell_forward && ' (next slate)'} · {games.length} game{games.length === 1 ? '' : 's'}
              {totalPicks > 0 && (
          ...
          {games.length === 0 ? (
            <div className="rounded-lg border border-dashed border-border bg-card/50 px-6 py-10 text-center">
              ...
              <div className="text-xs text-muted-foreground mt-1">
                NCAA D1 season runs February–June. Bracket games + College
                World Series push through mid-June.
              </div>
            </div>
          ) : (
            <>
          ...
          <div className="text-[10px] text-muted-foreground italic px-1">
            Beta — Elo + Monte Carlo + GBM ensemble fit on the
            current-season corpus. Run line and run total picks fire
            against HR's posted markets.
          </div>
        </>
      )}
    </div>
...
function BaseballGameCard({ game, bet, raw, sport }) {
  const { home, away } = game
  const odds = game.odds ? <BaseballOddsGrid odds={game.odds} home={home} away={away} /> : null
  const insight = <BaseballCardInsight bet={bet} home={home} away={away} raw={raw} />
  return (
    <GameCard
      game={game}
      bet={bet}
...
function BaseballOddsGrid({ odds, home, away }) {
  return (
    <OddsGrid>
...
function BaseballCardInsight({ bet, home, away, raw }) {
  const reasons = []
  const pred = raw?.prediction || {}

  if (pred.expected_margin != null && Math.abs(pred.expected_margin) >= 1.5) {
    const fav = pred.expected_margin > 0 ? home.abbreviation : away.abbreviation
    reasons.push({
      weight: Math.abs(pred.expected_margin) * 2,
      text: <><strong>{fav}</strong> projected +{Math.abs(pred.expected_margin).toFixed(1)} run margin</>,
    })
  }
  ...
  if (wp?.home != null && Math.abs(wp.home - 0.5) >= 0.15) {
...
function TrackerView({ api, leagueKey }) {
  ...
  const refresh = () => {
    setLoading(true)
    api.get(`/baseball/${leagueKey}/tracker/history`)
  ...
  const onSettle = () => {
    api.post(`/baseball/${leagueKey}/tracker/settle`)
      .catch(() => null)
      .finally(refresh)
  }
...
function CalibrationView({ api, leagueKey }) {
  ...
  useEffect(() => {
    setLoading(true)
    api.get(`/baseball/${leagueKey}/calibration`)
      .then(r => setData(r.data))
      .catch(() => setData(null))
      .finally(() => setLoading(false))
  }, [api, leagueKey])
    ...
    <section className="space-y-3">
      <div className="rounded-lg border border-border bg-card overflow-hidden">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-px bg-border">
          <Stat label="Avg total runs" value={fmt(c.league_avg_total)} />
          <Stat label="Home edge (runs)" value={fmt(c.home_advantage, '+')} />
