 ...
 * UnderdogNote
 * ──────────────────────────────────────────────────────────────
 * Short disambiguator rendered under the EdgeCallout when the
 ...
 * The bet is correct — the note just explains why.
 *
 * Silently renders nothing for non-ML picks, for favorite prices,
 * or when the model + market agree on who's favored.
 */
...
export default function UnderdogNote({ pick, wp, home, away }) {
  ...
  return (
    <div className="underdog-note">
      +EV underdog play: model gives {pickAbbr} <strong>{modelPct}%</strong>,
      market prices them at <strong>{impliedPct}%</strong>.
    </div>
  )
}
