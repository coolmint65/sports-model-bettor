"""Historical NHL closing-odds backfill from scoresandodds.com.
...
``data/nhl/historical_odds.db`` (separate DB so it doesn't bloat the
main nba.db and can be rebuilt without touching live state).

Coverage window: 2024-07-15 onward (earlier dates return 404). The
2023-24 NHL season is not available here — that gap is filed under
the V3.1 watchlist for later backfill via a paid odds API.

Usage::

    python -m scrapers.scoresandodds_nhl --start 2024-07-15 --end 2026-05-17
    python -m scrapers.scoresandodds_nhl --date 2025-01-15
    python -m scrapers.scoresandodds_nhl --gap-fill           # only days we don't already have

Powers V3.1 (market-as-feature) by giving the NHL GBM access to
...
_SCHEMA = """
CREATE TABLE IF NOT EXISTS nhl_historical_odds (
...
CREATE INDEX IF NOT EXISTS idx_nhlhist_date  ON nhl_historical_odds(date);
CREATE INDEX IF NOT EXISTS idx_nhlhist_teams ON nhl_historical_odds(home_abbr, away_abbr);
CREATE TABLE IF NOT EXISTS nhl_historical_odds_dates (
...
def _fetch(date_iso: str, timeout: float = 12.0) -> str | None:
    """Pull the day's NHL results page. None on 404 (out-of-window date)
    or any HTTP/network failure."""
    url = f"https://www.scoresandodds.com/nhl?date={date_iso}"
...
_EVENT_CARD_RE = re.compile(
    r'<div[^>]+id="nhl.(\d+)"[^>]+class="[^"]*event-card[^"]*"[^>]*>'
    r'(.*?)(?=<div[^>]+id="nhl.\d+"|<footer|\Z)',
    re.DOTALL,
)
...
def scrape_date(date_iso: str) -> dict:
    ...
    if body is None:
        ...
        conn = _conn()
        conn.execute(
            "INSERT OR REPLACE INTO nhl_historical_odds_dates "
    ...
    for event_id_str, card_html in cards.items():
        ...
        if not parsed.get("home_abbr") or not parsed.get("away_abbr"):
            continue
        conn.execute("""
            INSERT OR REPLACE INTO nhl_historical_odds (
...
def scrape_range(start_iso: str, end_iso: str,
    ...
    if gap_fill:
        rows = conn.execute(
            "SELECT date FROM nhl_historical_odds_dates"
        ).fetchall()
...
def _cli() -> int:
    ap = argparse.ArgumentParser(prog="scrapers.scoresandodds_nhl")
